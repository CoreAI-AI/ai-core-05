...
const sidebarItems = [
  { icon: MessageSquare, label: "New Chat", active: true, action: 'newChat' },
...
export const ChatSidebar = ({ chats, currentChat, onSelectChat, onNewChat, onSignOut, onOpenSettings, onDeleteChat, user }: ChatSidebarProps) => {
  return (
      ...
      <div className="p-4 border-b border-sidebar-border">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-sidebar-foreground">Multi-AI Chat</span>
          </div>
          <ThemeToggle />
        </div>
        ...
        <nav className="space-y-1">
          {sidebarItems.map((item) => (
            <Button
              ...
              onClick={item.action === 'newChat' ? onNewChat : item.action === 'settings' ? onOpenSettings : undefined}
            >
